...
class ConsentController(http.Controller):
    @http.route(
        "/contract/invite/code/<string:code>", type="http", auth="user", website=True
    )
    def contract_invite(self, code, **kwargs):
        consent_record = (
            request.env["family.member.consent"]
            .sudo()
        ...
        if (
            consent_record
            and consent_record.user_id
            and consent_record.user_id != request.env.user
        ):
        ...
        return request.render(
            "sale_generate_membership.contract_invite_consent", values
        )

    @http.route(
        "/contract/consent/accept/<string:code>",
        type="http",
        auth="user",
        methods=["POST"],
        website=True,
