...
class ProductAttributeCheckController(http.Controller):
    ...
    def check_attribute(self, selectedAttributeValue=None):
        ...
        if order and selectedAttributeValue:
            ...
            attribute = request.env['product.template.attribute.value'].sudo().browse(att_id)
    ...
    def get_updated_modal_content(self, counterValue=None):
        ...
        return request.env['ir.ui.view']._render_template(
            'website_sale_family_membership.modal_template',
...
class WebsiteSale(WebsiteSale):
    ...
    def cart_update(self, product_id, add_qty=1, set_qty=0, **kw):
        response = super(WebsiteSale, self).cart_update(product_id, add_qty, set_qty, **kw)
    ...
    def create_family_members(self, family_members_data_json):
        ...
        for member_data in family_members_data.values():
            ...
            new_partner = Partner.create({'firstname': member_data.get('firstname', ''), 'lastname': member_data.get('lastname', ''), 'email': member_data.get('email', '')})
